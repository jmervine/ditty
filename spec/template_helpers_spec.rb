...
describe TemplateHelpers do
  ...
  describe :post_contents do
    it "should return the contents of a post" do
      helpers.post_contents(File.join(CONFIG["store"], %w{2012 01 file1.txt})).should match /Contents for file1/
    end
  end
  ...
  describe :post_link do
    it "should return a link to a post" do
      helpers.post_link(File.join(CONFIG["store"], %w{ 2012 01 file1.txt })).should match /a href='\/post\/2012\/01\/file1.txt/
      helpers.post_link(File.join(CONFIG["store"], %w{ 2012 01 file1.txt })).should match /File1/
    end
  end
  describe :months do
